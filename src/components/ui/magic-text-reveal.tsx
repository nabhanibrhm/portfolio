...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  // Text dimensions — derived; recomputed only when text/font props change.
  ...
  const updateParticles = useCallback((
    ...
    particles.forEach(particle => {
      if (isHovered) {
        ...
        // Drift around the letter positions. As the reveal progresses, the
        // drift radius collapses to zero so the particles settle onto the text.
        const collapse = Math.max(0, 1 - revealProgress);
        const FR = FLOAT_RADIUS * collapse;
        ...
        // Calculate target position with random offset (shrinks as we settle)
        const randomOffsetX = FR * noiseX;
        const randomOffsetY = FR * noiseY;
        ...
        // Add dynamic jitter based on distance (also collapses as we settle)
        const distanceFromTarget = Math.sqrt(dx * dx + dy * dy);
        const jitterScale = FR > 0 ? Math.min(1, distanceFromTarget / (FR * 1.5)) : 0;
        const jitterX = (Math.random() - 0.5) * FLOAT_SPEED * jitterScale * collapse;
        const jitterY = (Math.random() - 0.5) * FLOAT_SPEED * jitterScale * collapse;

        particle.x += dx * TRANSITION_SPEED * deltaTime + jitterX;
        particle.y += dy * TRANSITION_SPEED * deltaTime + jitterY;

        // Contain particles within bounds with soft boundary (tighter as we settle)
        const distanceFromOrigin = Math.sqrt(
          Math.pow(particle.x - particle.originalX, 2) +
          Math.pow(particle.y - particle.originalY, 2)
        );
        if (distanceFromOrigin > FR) {
          const angle = Math.atan2(particle.y - particle.originalY, particle.x - particle.originalX);
          const pullBack = (distanceFromOrigin - FR) * (0.1 + 0.5 * revealProgress);
        ...
        // Bias toward full opacity as the reveal progresses
        if (revealProgress > 0) {
          particle.opacity =
            particle.opacity * (1 - revealProgress) + particle.originalAlpha * revealProgress;
        }
      }
    });
    ...
    const animate = (currentTime: number) => {
      ...
      if (revealStartRef.current != null) {
        revealProgress = Math.min(
          1,
          Math.max(0, (currentTime - revealStartRef.current) / revealDuration),
        );
      }
  ...
  }, [isHovered, showText, revealDuration, spread, speed, globalDpr, updateParticles, renderParticles]);
  ...
  useEffect(() => {
    const handleResize = () => {
      if (wrapperRef.current && textDimensions.width && textDimensions.height) {
        ...
        const basePadding =
          padding ?? (isMobile ? Math.max(fontSize * 0.3, 20) : Math.max(fontSize * 0.5, 40));

        const widthFloor = padding != null ? 0 : isMobile ? 120 : 200;
        const heightFloor = padding != null ? 0 : isMobile ? 60 : 100;
        const minWidth = Math.max(textDimensions.width + basePadding * 2, widthFloor);
        const minHeight = Math.max(textDimensions.height + basePadding * 2, heightFloor);
        ...
        // With an explicit padding (inline/text usage) size to the text itself —
        // clamping to the parent would cut wide text and collapse stacked heights.
        const finalWidth = padding != null ? minWidth : Math.min(minWidth, maxWidth);
        const finalHeight = padding != null ? minHeight : Math.min(minHeight, maxHeight);
  ...
  }, [textDimensions, fontSize, padding]);
  ...
  // Auto-reveal: once allowed to start (e.g. after the preloader), stamp the
  // start time so the animation loop can drive the particle → solid transition.
  useEffect(() => {
    if (!autoReveal || !revealStart) return;
    if (revealStartRef.current != null) return;
    if (!wrapperSize.width || !wrapperSize.height) return;
    revealStartRef.current = performance.now();
  }, [autoReveal, revealStart, wrapperSize.width, wrapperSize.height]);

  // The solid text crossfades in (and particles fade out) while revealing.
  const revealing = autoReveal ? revealStart : false;
  ...
  }, [autoReveal, resetOnMouseLeave, hasBeenShown]);
  ...
  return (
    ...
    >
      {/* Solid text — crossfades in as the particles settle (or on hover) */}
      <div
        ...
        style={{
          ...
          textAlign: 'center',
          opacity: showText || revealing ? 1 : 0,
          transition: `opacity ${revealing ? revealDuration : 200}ms ease-in`,
        }}
      >
        {text}
      </div>

      {/* Canvas for particle system — fades out as the solid text fades in */}
      <canvas
        ref={canvasRef}
        className="absolute top-0 left-0 w-full h-full"
        style={{
          opacity: revealing ? 0 : 1,
          transition: `opacity ${revealDuration}ms ease-in`,
        }}
      />
    </div>
  );
