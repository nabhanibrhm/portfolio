...
<!-- Shared accordion chrome (.index-list / .index-row / .idx-* / .panel*) lives
     in global.css. Below: the grid layout for this panel's stacked content
     (description, tag row, repo link) plus the tag/link styling. -->
<style scoped>
/* This panel stacks multiple blocks, so it lays them out as a grid (the shared
   base .panel-content is not a grid). */
.panel-content {
  display: grid;
  gap: 1.5rem;
}
